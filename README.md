...
**The use of SQL:**<br>
I used the MySQL RDBMS to transform and design the dataset into a Star Schema having two Fact Tables (Sales and Target table) and 5 denormalized dim tables, linking both of them to the Fact tables using foreign keys. while data transformation I used Recursive CTEs to fill in the missing dates in the Date Table, and CASE statements to change the data of Target Table so its not just a duplicate of Sales Table.
